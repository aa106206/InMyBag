...
SnapBag은 단순히 사진을 저장하는 앱이 아니라, 사진 속 객체를 이해하고 분리한 뒤 사용자의 하루 기록으로 다시 연결하는 AI 기반 흐름을 사용합니다.

### 물건 채집 파이프라인

사용자가 사진을 찍으면 Gemini가 이미지 속 물건을 인식하고, 인식된 물건 이름을 바탕으로 Grounding DINO가 객체 위치를 찾습니다. 이후 SAM2가 해당 영역의 객체를 분리해 배경이 제거된 물건 이미지로 변환합니다.

<p align="center">
  <img src="assets/readme/object-pipeline.png" alt="SnapBag 물건 채집 AI 파이프라인" width="900" />
</p>

### 그림일기 생성 파이프라인

이야기 기능은 사용자가 기록한 하루의 내용, 오늘의 물건, 감정 상태, 창의력 설정 등을 바탕으로 그림일기를 생성합니다. 텍스트 기반 이야기 구성에는 Gemini를 활용하고, 그림일기 이미지 생성에는 SDXL과 LoRA 파인튜닝을 활용해 SnapBag에 어울리는 그림체를 만들도록 설계했습니다.

<p align="center">
  <img src="assets/readme/story-pipeline.png" alt="SnapBag 그림일기 생성 AI 파이프라인" width="900" />
</p>
